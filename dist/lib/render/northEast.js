...
module.exports = function (_ref) {
  var parentContainer = _ref.parentContainer,
      ...
      verticalAlignment = _ref$verticalAlignmen === void 0 ? enums.verticalAlignment.BOTTOM : _ref$verticalAlignmen;
  ...
  var xTransform = Math.sin(toRadians(45)) * fontSize / 2;
  var yTransform = bounds.height;
  var textSelection = parentContainer.append('text').attr('transform', "translate(".concat(xTransform, ", ").concat(yTransform, "),rotate(-45)")).attr('x', 0).attr('y', 0).attr('dy', 0).style('text-anchor', 'start').style('font-family', fontFamily).style('font-weight', fontWeight).style('font-size', "".concat(fontSize, "px")).style('fill', fontColor).text(text);
